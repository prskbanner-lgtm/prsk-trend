...
// Node 18+ を想定。必要に応じて既存スクリプトをこのまま置き換えてください.
// 変更点: OUT_DIR -> public/data
...
if (!API_KEY) {
  console.error('Error: YOUTUBE_API_KEY が設定されていません。');
  process.exit(1);
}
...
function parseHistoryEntryDatetime(e) {
  ...
  if (e.date) return new Date(e.date + 'T00:00:00.000Z');
  return null;
}
...
(async () => {
  try {
    ...
    try {
      ...
      console.log('既存の public/data/videos.json が見つかりません。新規作成します。');
    }
    ...
    console.log('更新完了: public/data/videos.json を書き出しました。');
  } catch (err) {
